...
const WriteArticle = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try{
      ...
      const prompt = `write an article about ${input} with a length of around ${selectedLength.length} words. The article should be SEO-friendly and professional.`;
      ...
      if(data.success){
        ...
        toast.error("Failed to generate article" , data.message);
      }
    }
    catch (error){
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      >
        ...
        >
          {
            loading ? <span className='animate-spin'>🔄</span> : <Edit className='w-5'  />
          }
